    ...
    for (let i = top; i >= bottom; i--) {
      ...
      for (const tx of block.transactions) {
        ...
        if (v >= THRESHOLD_WEI) {
          totalCount += 1
          totalVolumeWei += v
          recent.push({ hash: tx.hash, from: tx.from, to: tx.to, valueEth: Number(ethers.formatEther(v)), timestamp: block.timestamp })
          hourly[hourIndex].count += 1
          hourly[hourIndex].volumeEth += Number(ethers.formatEther(v))
        }
      }
    }
  }
  const out = { summary: { totalCount, totalVolumeEth: Number(ethers.formatEther(totalVolumeWei || BigInt(0))) }, hourly, recent }
